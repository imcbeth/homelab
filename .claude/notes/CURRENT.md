...
2. ~~**Every ServiceAccount in the cluster can delete Calico network policies.**~~ **FALSE POSITIVE — retracted 2026-09-09.** `kubectl auth can-i` returns `yes`, but it only evaluates Kubernetes RBAC and is blind to the aggregated API server behind `projectcalico.org`. The real attempt is denied: `Forbidden: cannot delete globalnetworkpolicies in tier "default" (user cannot get tier)`. Calico's `calico-tiered-policy-passthrough` ClusterRole grants `system:authenticated` full CRUD **by design** — it is a passthrough, with authorization actually enforced on the `tiers` resource by the Calico API server. `get tiers` is denied to `system:authenticated`; `calico-tier-getter` is bound only to `system:kube-controller-manager`. The ClusterRole is owned by the Tigera operator (`blockOwnerDeletion: true`), so deleting it would only be recreated. **No action needed.**
