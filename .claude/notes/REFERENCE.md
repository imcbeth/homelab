...
| CRD schema drift the apiserver strips can never converge | Chart-rendered `properties: {}` inside a CRD openAPIV3Schema is REMOVED by the apiserver on persist, so desired != live forever and selfHeal retries indefinitely. Distinct from chart-label drift (selfHeal clears that in 30-60s). Symptom: app permanently OutOfSync on a one-line diff prefixed `>`. Fix: `ignoreDifferences` scoped to the specific CRD + jqPathExpression for that node — never the whole schema, or real changes get masked. Hit strimzi 1.2.0 on `kafkas.kafka.strimzi.io` (PR #887). Same family as the apiextensions/v1 defaulting gotcha | 2026-09-07 |
| Verify a version-support matrix from the FULL manifest, not a snippet | Nearly rejected strimzi 1.2.0 believing it dropped Kafka 4.2.1 — that read came from a truncated `STRIMZI_KAFKA_IMAGES` env dump showing only its first line (`4.2.0=...`). Grepping the whole rendered manifest showed 1.2.0 supports 4.2.0/4.2.1/4.3.0/4.3.1 — it ADDS a version and drops nothing. For any "does the new version still support X" question, extract the complete list (`helm template ... \| grep -oE '<pattern>' \| sort -u`) before concluding | 2026-09-07 |
| A watcher must not share a failure mode with what it watches | Obvious stated plainly, easy to violate in practice. Building a dead-man switch for the pvc-ro-remediator (which guards against iSCSI PVCs going read-only) disqualified every in-cluster candidate — Prometheus, Uptime Kuma, Grafana and Loki ALL sit on iSCSI PVCs, so each would have shared the exact failure being watched. The answer was a CronJob with no volumes at all: K8s API + DNS + SMTP only, emailing via curl's `smtps` (PR #904). When adding monitoring, enumerate the watcher's dependencies and check none overlaps the watched failure mode | 2026-09-07 |
| CFS throttling cannot be fixed by raising CPU limits | Throttling is accounted per 100ms period, so a burst-then-idle process wanting a full core for a few ms is throttled no matter how large the quota. Measured 2026-09-07: tripling node-exporter's limit dropped utilization 15%→4.7% and scrape time 801ms→205ms, but throttling only fell 51%→29%. Clearing a 25% threshold would need ~1000m on a process averaging 14m. Raise limits for real latency wins; do NOT expect the throttle percentage to follow. Alert on throttling AND utilization together | 2026-09-07 |
| Alert thresholds must match the cadence of what they watch | A flat 24h staleness threshold applied to a WEEKLY backup schedule (`0 3 * * 0`) fires ~6 days out of 7 while the backup succeeds on time. Split rules per cadence and match schedule names POSITIVELY (`schedule=~"velero-daily-.*"`) rather than negating — a newly added schedule then shows up as an un-alerted gap instead of silently inheriting the wrong threshold. Caught 2026-09-07 on the very alert resurrected from 198 days dormant (PR #889) | 2026-09-07 |
| `> 0` is the wrong threshold for security-finding alerts | `CriticalVulnerabilitiesDetected` used `sum by (image,namespace,resource)(...) > 0`, producing 51 permanently-firing alerts — one per workload with any critical CVE. Upstream base images always carry some criticals; that is steady state, not an incident, and the alert never resolves. Trivy exposes no `fixed_version` label so actionable-only filtering is impossible. Alert on CHANGE (`24h delta > 0`) and OUTLIERS (`per-image > 10`) instead. 51 permanent → 5 actionable + 1 regression signal (PR #890) | 2026-09-07 |
| Fixing a dormant alert is only half the job | After resurrecting a rule that never loaded, immediately check its threshold is correct for what it watches. The Velero delayed alert went from 198 days silent (2026-07-31) to firing 6 days a week (2026-09-07) — silence traded for noise, ignored either way. Verify a newly-loaded rule both LOADS and produces a sane firing rate | 2026-09-07 |
| trivy-operator reports outlive the workload | One VulnerabilityReport is written per ReplicaSet, and Kubernetes retains old ReplicaSets (`revisionHistoryLimit`, default 10). After three argocd upgrades in one day, reports existed for v3.4.5, v3.5.0 and v3.5.2 while only v3.5.2 was running — inflating CVE counts with undeployed images. Confirm an image is live before chasing it: `kubectl get pods -A -o jsonpath='{range .items[*]}{.spec.containers[*].image}{"\n"}{end}' \| sort -u \| grep <image>` | 2026-09-07 |
| Renovate prConcurrentLimit refills the queue as you drain it | `renovate.json` has `prConcurrentLimit: 10` + schedule `after 6am and before 9pm on saturday and sunday`. Inside that window Renovate tops the open-PR count back up to 10 every time you merge a batch — 2026-09-06 saw three waves (#850-859 → #864-874 → #876-882) in ~3 hours. It is a metered queue, not an infinite backlog. **Apply Renovate batches AFTER the weekend window closes (Mon-Fri)**, when the queue is static and you can see the true scope in one pass | 2026-09-06 |
| ArgoCD stale-repo-cache is the NORM for chart bumps, not an anomaly | Observed 14x+: gatekeeper + cert-manager (2026-07-14), kps 89.2.4 + argo-cd 10.8.1 (2026-09-06), then ALL TEN apps in the 2026-09-06 second wave simultaneously — including plain `base/` git-source files (cloudflared, pvc-ro-remediator), not just Helm chart sources. Every time, ArgoCD reported `Synced + Healthy` while the pod still ran the OLD image. Treat "verify the running pod image after a chart bump" as a mandatory step, not a spot check: `kubectl get deploy/sts <name> -o jsonpath='{.spec.template.spec.containers[0].image}'`. Fix is always `argocd.argoproj.io/refresh=hard` + explicit sync-patch | 2026-09-06 |
...
| An ignoreDeps entry only works if the depName matches EXACTLY | `ignoreDeps: ["synology-csi"]` did NOT stop Renovate — the actual depName for the image is `synology/synology-csi`. A wrong-named entry is silently ineffective; there is no warning. Verify by checking whether Renovate still opens PRs for the dep, and match the depName exactly as it appears in the PR title ("update **synology/synology-csi** docker tag"). Second instance of the "pin that doesn't pin" family (see the row below) | 2026-09-06 |
...
| PrometheusRule can exist for months without loading (3 instances, 7 rules) | A PrometheusRule resource being present in the cluster does NOT mean Prometheus loaded it. The velero-alerts rule sat in the `velero` namespace missing the `release: kube-prometheus-stack` label for 198 days — `/api/v1/rules` showed zero velero rules the whole time, so backup-failure alerts never fired. **2026-09-07: found SIX more** — blackbox-exporter, log-pipeline, network, pi-cluster, slo, storage. Undervoltage detection and the whole SLO burn-rate framework had never evaluated. Rule groups 55→70 after the fix (PR #896). ALWAYS verify a new/moved PrometheusRule appears in `kubectl exec prometheus -- wget -qO- localhost:9090/api/v1/rules`, not just that `kubectl get prometheusrule` lists it. A written note has now failed twice — enforce it in CI (TODO F12) | 2026-07-31 |
