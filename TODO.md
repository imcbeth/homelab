...
| F5 | **Flink UI uptime-kuma monitor points at a dead demo** | ⏸ **Needs a UI action — cannot be fixed via GitOps** | Diagnosis complete: `kafka-to-s3` is FAILED (`NoRestartBackoffTimeStrategy`), `file-to-kafka` is FINISHED, 0 pods, and the target Service only exists while a job runs. **The monitor is simply wrong for this workload** — it watches a batch demo as though it were a long-running service. **Blocked because uptime-kuma monitors live in its SQLite DB, not git** (only `prometheusrule.yaml` is version-controlled), and 1.x has no REST API for monitor CRUD. **Action: pause or delete the "Flink UI" monitor in the Uptime Kuma UI.** See F15 for the underlying gap. |
...
| F8 | **uptime-kuma image pinned to the 1.x line** | ⏸ **Needs your decision — deliberately not made unilaterally** | Running `1.23.17-debian`; chart appVersion is `2.5.0`. This is a **major upgrade with a data migration** (monitors, history and settings all live in the SQLite DB on the PVC), so it is not a change to make as a side effect of tidying a follow-up list. Two defensible options: **(a)** stay on 1.x and convert the pin from incidental to documented-and-intentional; **(b)** plan a 2.x migration with a fresh Velero backup and a tested rollback. Recommend (b) eventually — 1.23.x is receiving no fixes — but on its own change window. |
...
| **F15** | **Uptime Kuma monitors are unmanaged state** | ⬜ Open | Surfaced by F5. The monitor set lives only in uptime-kuma's SQLite DB on an iSCSI PVC — nothing is in git. Consequences: monitors cannot be reviewed or changed via GitOps, drift is invisible, and **losing the PVC loses every monitor definition**. The Velero daily-critical-pvcs backup does cover it, so this is recoverable rather than fragile, but it is the only significant piece of cluster config not under version control. Options: seed monitors from a ConfigMap at startup, or accept and document. |
