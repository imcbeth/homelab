...
| **F15** | **Uptime Kuma monitors are unmanaged state** | ⬜ Open — **now unblocked** | Surfaced by F5. Monitor definitions live only in uptime-kuma's SQLite DB — nothing in git, so they cannot be reviewed via GitOps and drift is invisible. **Correction:** an earlier version of this row claimed Velero covered the PVC. It did not — the namespace was in no schedule at all until PR #907 (2026-09-07). It is covered now. **CORRECTION 2026-09-09: 2.5.3 has NO monitor REST API.** Probed the running 2.5.3: `/api/monitors` and `/api/v1/monitors` return the SPA's `text/html`, byte-identical to a made-up control endpoint. Only `/api/entry-page` returns JSON. The claim that the 2.x migration unblocked this was wrong. Uptime Kuma's programmatic interface remains **socket.io**, not REST. So full GitOps reconciliation would need a socket.io client speaking an unofficial protocol — a much larger job than assumed. A cheaper option worth considering instead: periodic **drift detection** (export monitors from the SQLite DB, diff against a committed list, alert on change) — most of the value, none of the protocol risk. |
