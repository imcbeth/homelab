...
| | SynologySystemTemperatureHigh | warning | >50°C |
| | SynologyBadSectorsDetected | warning | Bad sectors detected |
| | SynologyPowerStatusAbnormal | critical | Power supply issue |

**Technical Details:**

**Predictive Alert Formula:**
```promql
# Predict filesystem full in 24 hours using linear regression
predict_linear(node_filesystem_avail_bytes{...}[6h], 24*3600) < 0
```

**Synology SNMP Metrics Used:**
...
- `raidFreeSize/raidTotalSize` - Volume capacity
- `temperature{job="snmp-synology"}` - System temperature
- `diskBadSector{job="snmp-synology"}` - Bad sector count
- `powerStatus{job="snmp-synology"}` - Power supply status

**Alert Routing:**
- Critical alerts → Email (via existing AlertManager SMTP config)
- Warning alerts → Null receiver (suppressed to reduce noise)

**Current State:**
- ✅ PrometheusRule `storage-alerts` deployed and validated
- ✅ 3 alert groups: `node_filesystem`, `persistent_volumes`, `synology_nas`
- ✅ All alerts registered in Prometheus
- ✅ Critical alerts route to email via existing AlertManager config

**For Next Session:**
- [ ] Monitor alerts for false positives (adjust thresholds if needed)
- [ ] Continue with TODO Section 8: Secrets Management evaluation
- [ ] Consider creating runbooks for storage alert scenarios
