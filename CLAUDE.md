...
Use `git rebase origin/main` (not merge) for conflict resolution. Verify the current branch and target remote, then use `git push --force-with-lease` for that branch only.
